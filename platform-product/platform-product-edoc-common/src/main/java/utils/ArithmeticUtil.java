...
public class ArithmeticUtil {
     ...
     * @return
     */
    public static BigDecimal round(BigDecimal args, int scale) {
        if (scale < 0) {
            throw new IllegalArgumentException(
                    "The scale must be a positive integer or zero");
        }
        ...
        return args.divide(one, scale, BigDecimal.ROUND_HALF_UP);
    }
     ...
     * @return 两个参数的积
     */
    public static double mul(double added, double augend,int scale) {
        BigDecimal addedBigDecimal = new BigDecimal(Double.toString(added));
        BigDecimal augendBigDecimal = new BigDecimal(Double.toString(augend));
     ...
     * @return 两个参数的积
     */
    public static BigDecimal mul(BigDecimal added, BigDecimal augend,int scale) {
        return  round(added.multiply(augend),scale);
    }
    ...
    /**
     * 提供（相对）精确的除法运算，当发生除不尽的情况时，精确到 小数点以后12位，以后的数字四舍五入。
     *
     * @param added
     *            被除数
     * @param augend
     ...
     * @return 两个参数的商
     */
    public static BigDecimal div(BigDecimal added, BigDecimal augend) {
        return div(added, augend, DEF_DIV_SCALE);
    }
     ...
     * @return 两个参数的商
     */
    public static BigDecimal div(BigDecimal added, BigDecimal augend, int scale) {
        if (scale < 0) {
            throw new IllegalArgumentException(
                    "The scale must be a positive integer or zero");
        }

        return added.divide(augend, scale, BigDecimal.ROUND_HALF_UP);
    }
     ...
     * @return 四舍五入后的结果
     */
    public static Double stringToDouble(String doubleVal, int scale) {
        try {
            BigDecimal decimal = new BigDecimal(doubleVal);
            return decimal.setScale(scale, BigDecimal.ROUND_HALF_UP).doubleValue();
        } catch (Exception e) {
            throw new NumberFormatException("The doubleVal for switched must be a positive number.");
        }
    }
}
