...
import com.zynsun.openplatform.exception.ParamInvalidException;
...
public class StringUtil {
    ...
    /**
     * 处理SAP中将负号置放在后面的金额  （ 123.12-     to   -123.12 ）
     *
     * @param sapNumber 源金额（123.12-）
     * @return 目标金额  （-123.12）
     */
    public static String parseSapNumberToNumber(String sapNumber) {
        sapNumber = StringUtils.trim(sapNumber);
        if (BeanUtil.isEmpty(sapNumber)) {
            return null;
        }
        if (sapNumber.endsWith("-")) {
            return "-" + sapNumber.split("-")[0];
        }
        return sapNumber;
    }
}
