...
public class ArithmeticUtil {
     ...
     * @return 两个参数的和
     */
    public static BigDecimal add(BigDecimal added, BigDecimal augend,int scale) {
        if (scale < 0) {
            throw new IllegalArgumentException("The scale must be a positive integer or zero");
        }

        return added.add(augend).setScale(scale, BigDecimal.ROUND_HALF_UP);
    }
     ...
     * @return 两个参数的积
     */
    public static BigDecimal mul(BigDecimal added, BigDecimal augend) {
        return added.multiply(augend);
    }
